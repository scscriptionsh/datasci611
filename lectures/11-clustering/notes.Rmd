...
I'll skip GMM to focus on what is probably the best currently available
general purpose clustering algorithm: spectral clustering.
...
have to run the python code in a separate step. Not sure what is
happening, frankly.
...
ggplot(data, aes(x,y)) + geom_point(aes(color=cluster));
...
Our data is from Guy Debord's "The Society of the Spectacle." This is a
somewhat polemical book about commodification in capitalism. While I'm
hardly a Marxist I do often find myself reflecting on "The Society of
the Spectacle" in this highly social-media influenced age.

After a bit of manual repair and fiddling I ripped the paragraphs from
the book into a data frame. We will create vectors from them by counting
words.
...
It can't hurt us to do a quick PCA to see whether anything obvious pops
out.
...
Our variance falls off relatively slowly, however. So we may not expect
to see anything of interest in the first two component projection, but
let's take a look anyway. In fact, here is a handy trick:
...
This isn't so easy to interpret. Let's take a look at the TSNE
projection.
